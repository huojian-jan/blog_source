...
所以计算机"存图片"这件事并不神秘——就是存一堆数字。

但问题是，这堆数字本身没有"猫"或者"表格"或者"代码截图"这些概念，它只是数字。**从一堆数字里推断出"这是一只猫"，才是 AI 理解图像要解决的核心问题。**

## 早期的解法：CNN

在 Transformer 出现之前，图像识别的主流方案是 **CNN（卷积神经网络）**。

CNN 的核心思路是"局部感知"：用一个小的滑动窗口（卷积核）在图片上扫描，每次只看一小块区域，提取局部特征。扫完一遍之后，这些局部特征再被逐层聚合成更抽象的特征。
...
关键在于**让图片的 embedding 和文字的 embedding 进入同一个向量空间**。

这里绕不开一个里程碑式的工作：**CLIP**（Contrastive Language-Image Pre-training），OpenAI 在 2021 年发布。

CLIP 的训练方式很直觉：给模型大量的"图片 + 文字描述"配对，训练的目标是让匹配的图文对在向量空间里靠近，不匹配的推远。
